 ...
 * SELECT 컬럼명, 컬럼명,...
 * FROM 테이블명;
 * -> 지정된 테이블에서 모든 행의 컬럼명이 일치하는 컬럼값 조회
 * */
-- EMPLOYEE 테이블에서 모든 행의 사번(EMP_ID), 이름(EMP_NAME), 급여(SALARY)조회
SELECT EMP_ID, EMP_NAME, SALARY FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 모든 행의 이름(EMP_NAME), 입사일(HIRE_DATE)조회
...
-- SYSDATE : YY:MM:DD TT:MM:SS 조회(ms 기록 안 함)
-- SYSTIMESTAMP :  YY:MM:DD TT:MM:SS.sss 조회(ms 기록 함) + 지역(local) 조회
...
-- 단, 컬럼명는 작성된 그대로 조회
SELECT EMP_ID AS 사번, EMP_NAME AS "사원 이름", SALARY 급여, SALARY * 12 AS "연봉(급여 * 12)" FROM EMPLOYEE;
...
/* DISTINCT(별개의, 전혀 다른) */
--> 중복 제거
-- 조회 결과 집합(RESULT SET)의 지정된 컬럼의 값이 중복되는 경우
-- 이를 한 번만 표시할 때 사용
...
-- 사번, 이름 연봉 조회
SELECT EMP_ID, EMP_NAME, SALARY * 12  FROM EMPLOYEE WHERE SALARY * 12 < 50000000;
