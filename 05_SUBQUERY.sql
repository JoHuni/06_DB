...
/* 1) 메인 쿼리 1행의 값을 서브쿼리로 전달
 * 2) 서브 쿼리에서 전달 받은 값을 이용해서 SELECT를 수행
...
-- WHERE EXISTS (서브쿼리)
	--> 서브쿼리 결과(RESULT SET)에 행이 존재 한다면 TRUE, 없으면 FALSE
...
ORDER BY SALARY DESC;
-- [문제 발생] --
--> SELECT절이 ORDER BY보다 먼저 해석 되어
-- ROWNUM 순서가 급여가 아닌, 조회된 행 순서로 지정됨
/* 인라인 뷰를 이용해서 문제 해결 */
...
-- 급여 평균이 3위 안에 드는 부서의 
-- 부서코드와 부서명, 평균급여를 조회
SELECT ROWNUM, DEPT_CODE, DEPT_TITLE, "평균 급여" /* 인라인 뷰에 사용된 평균 급여 */
FROM(
	/* 인라인 뷰 */
...
SELECT EMP_ID, EMP_NAME, JOB_NAME, EXTRACT (YEAR FROM SYSDATE)
- (19||SUBSTR(EMP_NO, 1, 2)) 나이,
	TO_CHAR(SALARY * (1 + NVL(BONUS, 0)) * 12, 'L999,999,999,999') AS "보너스포함연봉"
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE)
WHERE (JOB_CODE, SUBSTR(EMP_NO, 1, 2)) IN (
	SELECT JOB_CODE, MAX(SUBSTR(EMP_NO, 1, 2))
	FROM EMPLOYEE SUB
	GROUP BY JOB_CODE)
ORDER BY 나이 DESC;
