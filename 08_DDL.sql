...
--    USER_ID VARCHAR2(20) UNIQUE, -- 컬럼 레벨 제약 조건 설정 (이름 X)
    USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, -- CONSTRAINT 제약조건명 제약조건
    												  -- 컬럼 레벨 제약 조건(이름 O)
    ...
    EMAIL VARCHAR2(50),
    -- 테이블 레벨 --
--	UNIQUE(USER_NAME) -- 테이블 레벨 설정 제약 (이름 X)
	CONSTRAINT USER_NAME_U UNIQUE(USER_NAME) -- 이름 O
);
...
-- 4. FOREIGN KEY(외부키 / 외래키) 제약조건 * !ㅈㄴ 2번째로 중요! *
...
-- SQL Error [2291] [23000]: ORA-02291: 무결성 제약조건(KH_JHH.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
--> 참조하는 테이블에 값이 존재하지 않음
...
SELECT * FROM USER_USED_FK2; -- 10을 참조하던 테이블이 NULL로 변함 -> ON DELETE SET NULL때매
...
CREATE TABLE USER_USED_FK3(
  ...
  ON DELETE CASCADE -- 부모키 삭제시 참조하고 있던 자식의 행도 모두 삭제
);
...
CREATE TABLE USER_TEST(
	...
	PNO VARCHAR2(20) CONSTRAINT UK_PNO UNIQUE
	CONSTRAINT NN_PNO NOT NULL,
	GENDER CHAR(1) CONSTRAINT CK_GENDER CHECK(GENDER IN ('남', '여'))
);
