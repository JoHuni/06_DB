...
-- substr은 시작 위치가 1부터 시작 0부터 시작이 아님
...
-- (조회 결과가 사각형 형태의 테이블 모양이면 조회가 가능하다)
...
FROM EMPLOYEE;
-- 6행만 나옴 -> COUNT 내에 컬럼명 작성하면 NULL은 제외되기 때문.
...
SELECT COUNT(DECODE(SUBSTR(EMP_NO, 8, 1), '2', '여자', NULL)), 
	   COUNT(DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남자', NULL)) 
FROM EMPLOYEE;
