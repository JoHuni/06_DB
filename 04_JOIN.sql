...
-- 사번 오름차순으로 조회
/*3*/SELECT EMP_ID 사번, EMP_NAME 이름, DEPT_TITLE 부서명
/*1-1*/FROM EMPLOYEE
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
--> 하동운, 이오리가 누락됨
--? WHY? DEPT_CODe(NULL)값과
	-- 같은 값이 DEPT_ID 컬럼에 존재하지 않아서
	-- 연결되지 않음 --> 최종적으로 결과에서 제외됨 == INNER JOIN
...
--> 모든 경우의 수를 보고 싶을 때 빼고는 직접 작성하는 경우가 없음
	--> 자연 조인 실패 결과로 교차 조인 결과가 출력되는 경우가 있음
		-- (교차 조인 결과가 보이면 -> ㅅㅂ 자연 조인 잘못 썼네)
SELECT EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE
...
-- 각 사원의 이름, 사수 이름 조회
-- ANSI 표준
SELECT E1.EMP_NAME 부사수, E2.EMP_NAME 사수
FROM EMPLOYEE E1
...
-- JOB.JOB_CODE       (CHAR(2))
	--> 컬럼명, 자료형이 모두 일치! == NATURE JOIN의 대상
-- NATURAL 조인 안 쓰고 ㄱㄱ
...
SELECT E1.EMP_NAME 사원명, E2.DEPT_CODE 부서코드, E2.EMP_NAME 동료이름
FROM EMPLOYEE E1
